...
from src.architectures.configuration import OnlyDecoderConfiguration
import os
import time
import numpy as np
import torch
...
class OnlyDecoderTransformerLightning(BaseArchitecture, L.LightningModule):
  def __init__(self, config: OnlyDecoderConfiguration):
    # Setup architecture
    BaseArchitecture.__init__(self)
    L.LightningModule.__init__(self)
    self.model = DecoderOnlyTransformer(config.num_inputs, config.num_outputs, config.d_model, config.num_heads, config.num_layers, config.dropout)
  ...
  def training_step(self, batch, batch_idx):
    ...
    output = self.model(data, target).squeeze().float()
...
class DecoderOnlyTransformer(nn.Module):
  def __init__(self, num_inputs: int, num_outputs: int, d_model: int, num_heads: int, num_layers: int, dropout: float):
    super().__init__()
    
    self.linear_features = nn.Linear(num_inputs, d_model)
    self.linear_target = nn.Linear(num_outputs, d_model)
  ...
  def forward(self, features, targets):
    ...
    targets = self.linear_target(targets)

    features = features.permute(1, 0, 2)  # (sl, B, d_model)
    targets = targets.permute(1, 0, 2)    # (fl, B, d_model)

    output = self.transformer_decoder(targets, features)
    output = self.output_layer(output[-1])  # (B, 1)
